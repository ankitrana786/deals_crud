...
export default function ListBox({ options }) {
	...
	let name = searchParams.get("sort_price");
	// console.log(name);
	...
	function handleItemClick(values) {	
		...
		if(values.value == ''){
			...
			const param = searchParams.get('sort_price');
			// console.log(param);
			if (param) {
				param.delete('sort_price');
		...
		// const { sort_price, ...restQuery } = query;
		// router.push(
		// 	{
		// 		pathname,
		// 		query: {
